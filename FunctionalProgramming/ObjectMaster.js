...
// console.log("FireType", FireType)
const MoreThanOneType = pokemon.filter(p => p.types.length > 1)
// console.log("MoreThanOneType", MoreThanOneType)
const OnlyNames = pokemon.map(p => p.name)
// console.log("OnlyNames", OnlyNames)
const IdBiggerThanNightyNine = pokemon.filter(p => p.id > 99)
// console.log("IdBiggerThanNightyNine", IdBiggerThanNightyNine)
const OnlyPoison = pokemon.filter(p => p.types.length === 1 && p.types.includes("poison")).map(p => p.name)
// console.log("OnlyPoison", OnlyPoison)
const SecondTypeFlying = pokemon.filter(p => p.types[1] == "flying").map(p => p.types[0])
// console.log("SecondTypeFlying", SecondTypeFlying)
